...
export class UserService {
  ...
  async updateUserAvatar(avatarUrl?: File | null) {
    ...
    return await this.http.patch<TSuccessResponse<TUser>>('/users/update_user_avatar', fd, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
  ...
  async updatePassword({password, newPassword}: TUpdateUserPassword) {
    return await this.http.patch<TSuccessResponse<TUser>>('/users/update_user_password', { password, new_password: newPassword })
  }

  async deleteAccount() {
    return await this.http.delete<TSuccessResponse<null>>('/users/delete_user')
  }
}